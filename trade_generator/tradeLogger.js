...
tradeLogger.logClosedTrade = function(pair, units, openPrice, closePrice, direction, timestamp, db){
  var directionNum;
  if(direction){
    directionNum = 1;
  }else{
  ...
  var profit = 50 * units * (closePrice - openPrice) * directionNum;

  var doc = {type: "close", timestamp: timestamp, pair: pair, units: units, closePrice: closePrice, direction: direction};
  db.collection("tradeHistory").insertOne(doc, function(err, res){
