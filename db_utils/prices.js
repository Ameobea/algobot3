...
priceUtils.getPricesInRange = (pair, startTime, endTime, getBefore, db, callback)=>{
  var prices = db.collection("prices");
  prices.find({pair: pair, timestamp: {$gte: startTime, $lte: endTime}}).toArray((err, res)=>{
    if(err){
      console.log("Error in prices.js line 14: ", err);
    }
    if(getBefore){
      prices.find({timestamp: {pair: pair, $lt: startTime}}).sort({timestamp: -1}).limit(1).toArray((err, prev)=>{
        if(err){
